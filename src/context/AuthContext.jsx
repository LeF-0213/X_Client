...
export function AuthProvider({ authService, authErrorEventBus, children }) {
  ...
  const signUp = useCallback(
    async (userid, password, name, email, url) =>
      authService.signup(userid, password, name, email, url).then((user) => setUser(user)),
    [authService]
  );
  const logIn = useCallback(
    async (userid, password) => authService.login(userid, password).then((user) => setUser(user)),
    [authService]
  );
